...
export class Ec2Stack extends Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // --------------------------------------------------------------
    // 기존 CF 스택이 VPC ID를 Outputs로 export한다고 가정 (ExportName= "PoCVpcIdExport")
    // 아래처럼 importValue로 가져온다:
    // --------------------------------------------------------------
    const existingVpcId = cdk.Fn.importValue('PoCVPCIDExport'); // <-- 기존 VPC ID를 import

    // 4) 해당 VPC의 서브넷들(퍼블릭/프라이빗)도 같이 알아야 Instance를 만들 수 있음
    ...
    // --------------------------------------------------------------
    const myExistingVpc = ec2.Vpc.fromVpcAttributes(this, 'PoCVpc', {
      vpcId: existingVpcId,
      availabilityZones: [ 'ap-northeast-2a', 'ap-northeast-2b' ], // 실제 AZ
      // 아래는 예시. subnet Id를 knownSubnets로 등록할 수도 있음:
      // publicSubnetIds: [ cdk.Fn.importValue('MyPublicSubnetId1'), cdk.Fn.importValue('MyPublicSubnetId2') ],
